...
function loadPageScript() {
  ...
  for (const file of ["sub2api.js", "converter.js"]) {
    vm.runInContext(
...
function dispatch(element, type) {
  assert.equal(
    typeof element.listeners[type],
    "function",
    `missing ${type} listener on ${element.selector}`,
  );
...
function testSub2apiAccountWithRefreshTokenOmitsAccessTokenExpiry() {
  ...
  assert.equal(account.credentials.expires_at, "2026-06-03T08:06:00.000Z");
...
function testSyntheticIdTokenHasCodexParseableJwtFormat() {
  ...
  const cpaButton = formatButtons.find(
    (button) => button.dataset.format === "cpa",
  );
  const input = elements.get("#session-input");
  const output = elements.get("#output");
...
function testAxonHubAuthJsonUsesPlaceholderRefreshTokenWhenMissing() {
  const { elements, formatButtons } = loadPageScript();
  const axonHubButton = formatButtons.find(
    (button) => button.dataset.format === "axonhub",
  );
  const input = elements.get("#session-input");
  const output = elements.get("#output");
...
function testCodexAuthJsonMatchesNativeShapeWhenMissingRefreshToken() {
  const { elements, formatButtons } = loadPageScript();
  const codexButton = formatButtons.find(
    (button) => button.dataset.format === "codex",
  );
  const input = elements.get("#session-input");
  const output = elements.get("#output");
...
function testCodexManagerAuthJsonUsesEmptyRefreshTokenWhenMissing() {
  const { elements, formatButtons } = loadPageScript();
  const codexManagerButton = formatButtons.find(
    (button) => button.dataset.format === "codexmanager",
  );
  const input = elements.get("#session-input");
  const output = elements.get("#output");
